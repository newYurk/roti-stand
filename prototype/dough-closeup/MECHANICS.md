...
> Обновлено 25.08.2026, сборка «тава и запятая». Это описание того, что реально
...
- У узла копится `cook`: скорость = жар места (вогнутое дно: центр с лужицей масла горячее
  края) × тонкость (тонкое золотится первым). Цвет: сырое → золотистое → коричневое → тёмное.
