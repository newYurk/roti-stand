...
- У узла копится `cook`: скорость = **тонкость** (время до цвета ∝ толщине; главный член)
  × жар места (широкое горячее пятно, борт ≈ 0,7) × масло (+30 % в лужице). До фактчека
  26.08.2026 стенд грел «из центра» (центр 1,0 → край 0,25) — это было домыслом, см.
  `docs/frying-mechanics.md` §1. Цвет: сырое → золотистое → коричневое → тёмное.
- Пресет шага 3 даёт **профиль настоящего листа**: радиальная карта r′ = R·(0,7ρ + 0,3ρ³),
  середина ≈ в 3 раза толще края (по рецептам: «edge paper-thin, center thicker»).
  Слепленный шлепками лист получает свой профиль сам — веса удара тянут край сильнее.
