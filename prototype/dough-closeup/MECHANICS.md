...
> Обновлено 16.09.2026, сборка «конверт и нарезка · 1». Это описание того,
...
**Ограничение среза:** после посадки геометрическая проба приостанавливает прежнюю заглушку
жарки и её постоянный звук. Температура, две стороны, переворот, дожаривание, настоящий
жест снятия, украшение и карточка результата ещё не подключены. Переход к нарезке не
объявляет блюдо приготовленным. #20/#21/#53 остаются открытыми.
...
- **Размером лист не ограничен** (⚑ решение владельца 08.09.2026): недотянутый несётся на таву
  так же, как готовый. Запрета нет и предупреждения нет — целевой эффект: толстый лист
  медленнее сохнет и иначе набирает цвет; в текущей пробе конверта жарка приостановлена. Строка решения называет размер в момент броска.
...
## Жарка: сохранённая заглушка, приостановлена в пробе конверта

Ниже устройство прежней заглушки. В сборке «конверт и нарезка · 1» после посадки работает
геометрическая проба выше: `step()` не вызывает `cookStep()`, постоянный звук выключен.
