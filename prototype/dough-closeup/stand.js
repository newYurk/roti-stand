...
const BUILD = "2026-09-17 · объём конверта · 12";
...
function startDish(){
  ...
  for(let id=0;id<7;id++){
    ...
    for(const face of base){
      ...
      if(p.length){ faces.push({points:p,kind:"filling",source:id,tone:id%3,turned:false,pc:{x,y},pr:radius,ph:FILL_MM}); added=true; }
    }
...
// ─── Толщина в миллиметрах и объём начинки (решение владелицы 17.09: всё влияет и на жарку,
...
function drawDishSections(g,screen,faces,sx,sy,zk=0){
  ...
  for(const s of walls){
    ...
    const weights=zk ? w.stack.map(f=>Math.max(f.kind==="filling" ? unit*1.5 : unit, layerMM(f,mid)*zk))
...
// ─── Вид толщины (решение владелицы 17.09). Условности отрисовки, не физика:
...
// Высота стопки под каждой гранью и верх поверхности на сетке 48×48. Грани идут снизу
// вверх; грань поднимается на высоту того, что уже лежит под её центром. Кэш — по массиву
// граней, как у торца: снимок геометрии неизменяем.
function stackHeights(faces){
  ...
  const top=new Float32Array(size*size), zb=new Float32Array(faces.length), zt=new Float32Array(faces.length);
  ...
  const layers=new Array(size*size), bx=new Float64Array(faces.length*4);
  ...
  for(let i=0;i<faces.length;i++){
    ...
    // Под гранью — только то, что действительно накрывает её центр: соседний квадрат того же
    // слоя лишь касается ячейки, и считать его «снизу» значит поднять грань на пустом месте.
    const c={x:cx/ps.length,y:cy/ps.length}, gi=ci(c.x), gj=cj(c.y), seen=new Set();
    let base=0;
    for(let j=Math.max(0,gj-1);j<=Math.min(size-1,gj+1);j++) for(let k=Math.max(0,gi-1);k<=Math.min(size-1,gi+1);k++){
      for(const e of layers[j*size+k]||[]){
        if(seen.has(e.idx) || e.key===key) continue; seen.add(e.idx);
        const o=e.idx*4, pad=1e-7;   // рамка отсекает соседей до точного теста
        if(c.x<bx[o]-pad || c.x>bx[o+1]+pad || c.y<bx[o+2]-pad || c.y>bx[o+3]+pad) continue;
        const g=faces[e.idx]; if(pointInFace(g.points,c.x,c.y)) base+=layerMM(g,c);
      }
    }
    ...
    for(let j=cj(y0);j<=cj(y1);j++) for(let k=ci(x0);k<=ci(x1);k++){
      ...
      const h=layerMM(f,q); list.push({key,h,idx:i}); top[cell]+=h;
    }
  }
  const out={zb,zt,top,L,T,dx,dy,size}; cache.set(faces,out); return out;
}
...
// перевёрнутой стопкой поверх того, что лежит в месте назначения. Приближение для
// движения пальца; после отпускания считается точно.
function previewHeights(faces){
  ...
  const out={zb,zt,top:H.top,L:H.L,T:H.T,dx:H.dx,dy:H.dy,size:H.size}; cache.set(faces,out); return out;
}
// Свет сверху-слева-спереди на склоне поверхности: множитель яркости грани (0,72…1,22).
function stackLight(H,x,y){
  const n=H.size, i=Math.min(n-2,Math.max(1,Math.floor((x-H.L)/H.dx))), j=Math.min(n-2,Math.max(1,Math.floor((y-H.T)/H.dy)));
  const s=Z_EXAG/ROTI_R_MM;
  const gx=(H.top[j*n+i+1]-H.top[j*n+i-1])/(2*H.dx)*s, gy=(H.top[(j+1)*n+i]-H.top[(j-1)*n+i])/(2*H.dy)*s;
  const lx=-.42,ly=-.5,lz=.76, len=Math.hypot(gx,gy,1);
  return Math.max(.72,Math.min(1.22,((-gx*lx-gy*ly+lz)/len)/lz));
}
function drawDishFaces(g,faces,screen,H,zk){
  // Тень приподнятого и «юбка» — боковая сторона стопки: каждая грань ещё раз на высоте
  // своего основания (у начинки — и посередине) тёмным цветом стенки, чтобы перепад высот
  // между стопками закрывала сторона, а не щель с видом на таву. Отдельные заливки мелких
  // треугольников: один общий путь из тысяч наложенных контуров растеризуется в разы дольше.
  if(H){
    const poly=(ps,dx,off)=>{
      const p=screen(ps[0]); g.beginPath(); g.moveTo(p.x+dx,p.y-off);
      for(let k=1;k<ps.length;k++){ const q=screen(ps[k]); g.lineTo(q.x+dx,q.y-off); }
      g.closePath(); g.fill();
    };
    g.fillStyle="rgba(18,10,4,.34)";
    for(let i=0;i<faces.length;i++) if(H.zb[i]>.3) poly(faces[i].points,zk*H.zt[i]*.25,0);
    for(const [kind,color] of [["dough","rgb(150,126,88)"],["filling","rgb(168,128,52)"]]){
      g.fillStyle=color;
      for(let i=0;i<faces.length;i++){
        const f=faces[i]; if(f.kind!==kind || H.zb[i]<.3) continue;
        poly(f.points,0,H.zb[i]*zk);
        if(kind==="filling") poly(f.points,0,(H.zb[i]+H.zt[i])/2*zk);
      }
    }
  }
  for(let i=0;i<faces.length;i++){
    ...
    const off=H ? H.zt[i]*zk : 0;
    if(H){
      const ps=f.points; let cx=0,cy=0; for(const p of ps){cx+=p.x;cy+=p.y;}
      const k=stackLight(H,cx/ps.length,cy/ps.length); color=color.map(c=>Math.min(255,c*k));
    }
    g.fillStyle=rgb(color);
    g.globalAlpha=f.kind==="filling" ? 1 : Math.max(.48,Math.min(.92,.48+doughMM(f.tone)*.3));
    const p=screen(f.points[0]); g.beginPath();g.moveTo(p.x,p.y-off);
    for(let k=1;k<f.points.length;k++){const q=screen(f.points[k]);g.lineTo(q.x,q.y-off);}
    g.closePath();g.fill();
  }
  g.globalAlpha=1;
}
...
function drawDish(g,sx,sy){
  ...
  if(!moving&&(dish.folds||dish.cuts.length)&&!(dishGesture&&dishGesture.preview)) drawDishSections(g,screen,faces,sx,sy,zk);
