...
// Щель ножа видна и там, где рез не разделил тесто (надрез): вдоль пропила, где по обе стороны
// одно и то же тесто, рисуется тёмная линия на высоте поверхности (проба 17.09: из трёх резов
// читался один). Где куски разошлись, щель и так видна.
function dishSlits(faces){
  const cache=dishSlits.cache||(dishSlits.cache=new WeakMap());
  if(cache.has(faces)) return cache.get(faces);
  const piece=dishPieces(faces), boxes=faces.map(f=>{
  ...
  const pieceAt=q=>{
    for(let i=0;i<faces.length;i++){
      const b=boxes[i]; if(faces[i].kind!=="dough" || q.x<b[0]||q.x>b[1]||q.y<b[2]||q.y>b[3]) continue;
      if(pointInFace(faces[i].points,q.x,q.y)) return {piece:piece[i],face:faces[i]};
    }
    return null;
  };
  const runs=[];
  for(const c of dish.cuts){
    const dx=c.b.x-c.a.x, dy=c.b.y-c.a.y, len=Math.hypot(dx,dy)||1, ux=dx/len, uy=dy/len, nx=-uy, ny=ux, off=c.width/2+.006;
    let run=null;
    const n=Math.max(2,Math.ceil(len/.015));
    for(let k=0;k<=n;k++){
      const t=len*k/n, m={x:c.a.x+ux*t,y:c.a.y+uy*t};
      const L=pieceAt({x:m.x+nx*off,y:m.y+ny*off}), R=pieceAt({x:m.x-nx*off,y:m.y-ny*off});
      if(L && R && L.piece===R.piece){ if(!run){ run={face:L.face,pts:[]}; runs.push(run); } run.pts.push(m); }
      else run=null;
    }
  }
  const out=runs.filter(r=>r.pts.length>1);
  cache.set(faces,out); return out;
}
function drawDishSlits(g,screen,faces,H,zk,sx,sy){
  if(!dish.cuts.length) return;
  const shift=dishPieceShift(faces), pixel=Math.min(2,devicePixelRatio||1)*Math.min(sx,sy);
  for(const r of dishSlits(faces)){
    const o=shift && shift.get(r.face) || {x:0,y:0};
    const pts=r.pts.map(p=>{ const q=screen({x:p.x+o.x,y:p.y+o.y}); return {x:q.x,y:q.y-(H ? H.at(p)*zk : 0)}; });
    const line=(w,style,dy)=>{ g.strokeStyle=style; g.lineWidth=w; g.beginPath(); g.moveTo(pts[0].x,pts[0].y+dy); for(let i=1;i<pts.length;i++) g.lineTo(pts[i].x,pts[i].y+dy); g.stroke(); };
    line(Math.max(1.4,2.2*pixel),"rgba(26,15,6,.82)",0);
    line(Math.max(.6,.8*pixel),"rgba(255,236,190,.45)",Math.max(1,1.4*pixel));
  }
}
...
function drawDish(g,sx,sy){
  ...
  if(!moving && dish.cuts.length && faces===dish.faces) drawDishSlits(g,screen,faces,H,zk,sx,sy);
