...
const BUILD = "2026-09-17 · поворот на iPhone · 28";
...
function undoDish(){
  if(dishTwist) endTwist(false);            // кнопка посреди поворота: поворот не считается
...
function rotateDish(angle,pivot=twistPivot()){
  ...
  if(dish.mode==="cut"){
    ...
    dish.history.push({faces:dish.faces,folds:dish.folds,cuts:dish.cuts,flips:dish.flips||0,cutBounds:dish.cutBounds});
    if(dish.history.length>20) dish.history.shift();
    dish.faces=faces; dish.cuts=dish.cuts.map(c=>({...c,a:R(c.a),b:R(c.b)}));
    dish.cutBounds=dishBounds();
  } else {
    panPress=[]; popAllDomes();
...
function syncDishUI(){
  ...
  if(dish) hintEl.textContent=dish.mode==="fold"
    ? "край: внутрь — складка · мах — переворот · на стол — снять · держи — прижим"+(dish.folds ? " · два пальца — повернуть" : "")
    : dish.mode==="served" ? "Подано · «следующий роти» — новый кусочек"
    : "Нарезка: проведи через конверт прямым жестом · два пальца или колесо — повернуть · «подать» — когда готово";
}
...
function drawDishSections(g,screen,faces,sx,sy,zk=0,H=null){
  ...
  const build=path=>{
    for(const f of faces){
      ...
      for(let i=1;i<ps.length;i++){const p=screen(ps[i],f);path.lineTo(p.x,p.y);}path.closePath();
    }
  };
...
const slabTop = (H,p) => Math.min(SLAB_MM, Math.max(0, H.at(p))*SLAB_GAIN);
...
const TABLE_K = 8;                     // сколько слоёв помнит пиксель
const TABLE_RELIEF = 0.25;             // доля мелкого рельефа верха (остальное — крупно сглажено)
let tableCanvas = null;
function tableVertexData(faces,H,zk){
  // Цвет вершины — среднее цветов граней того же слоя в этой точке: иначе верх — мозаика
  // квадратиков сетки жарки. Свет подушки — вполсилы, верх куска почти ровный.
  const key=(p,f)=>Math.round(p.x*2e4)+","+Math.round(p.y*2e4)+(f.turned ? "t" : "f");
  const acc=new Map();
  for(const f of faces){
    if(f.kind!=="dough") continue;
    const c=dishCookColor(f);
    for(const p of f.points){ const k=key(p,f), a=acc.get(k); if(a){ a[0]+=c[0]; a[1]+=c[1]; a[2]+=c[2]; a[3]++; } else acc.set(k,[c[0],c[1],c[2],1]); }
  }
  return faces.map(f=>{
    const hs=new Float32Array(f.points.length), cs=new Float32Array(f.points.length*3);
    f.points.forEach((p,m)=>{
      hs[m]=(slabTop(H,p)+(f.kind==="filling" ? .3 : 0))*zk;
      let c;
      if(f.kind==="filling") c=BANANA_FLESH[f.source%3];
      else { const a=acc.get(key(p,f)); c=[a[0]/a[3],a[1]/a[3],a[2]/a[3]]; }
      const l=1+((H.lightPoint ? H.lightPoint(p) : 1)-1)*.5;
      cs[m*3]=c[0]*l; cs[m*3+1]=c[1]*l; cs[m*3+2]=c[2]*l;
    });
    return {hs,cs};
  });
}
...
function renderDishTable(faces,screen,H,zk,W,Hc,s){
  const t0=performance.now(), n=faces.length, shift=dishPieceShift(faces);
  const P=new Array(n);
  let X0=Infinity,X1=-Infinity,Y0=Infinity,Y1=-Infinity;
  for(let i=0;i<n;i++){
  ...
  const N=bw*bh, K=TABLE_K, V=tableVertexData(faces,H,zk);
  const tf=new Int32Array(N).fill(-1), th=new Float32Array(N), tu=new Float32Array(N), tv=new Float32Array(N);
  const col=new Float32Array(N*3), pf=new Int32Array(N*K), pn=new Uint8Array(N), pd=new Uint8Array(N);
  const layer=new Int32Array(n); { const m=new Map(); faces.forEach((f,i)=>{ const k=f.kind+":"+f.source+":"+f.turned; if(!m.has(k)) m.set(k,m.size); layer[i]=m.get(k); }); }
  // Грани идут снизу вверх: последняя накрывшая пиксель — его верх.
  for(let i=0;i<n;i++){
    const q=P[i], ps=faces[i].points, {hs,cs}=V[i], dough=faces[i].kind==="dough";
    for(let t=1;t+1<q.length;t++){
      const A=q[0], B=q[t], C=q[t+1];
      const area=(B.x-A.x)*(C.y-A.y)-(B.y-A.y)*(C.x-A.x);
      if(Math.abs(area)<1e-9) continue;
      // Барицентрики линейны по x: считаем на начале отрезка строки и шагаем.
      const inv=1/area, dax=-(C.y-B.y)*inv, dbx=-(A.y-C.y)*inv;
      const y0=Math.max(Y0,Math.floor(Math.min(A.y,B.y,C.y))), y1=Math.min(Y1-1,Math.ceil(Math.max(A.y,B.y,C.y)));
      for(let y=y0;y<=y1;y++){
        const py=y+.5;
        ...
        if(xa>xb) continue;
        let la=((C.x-B.x)*(py-B.y)-(C.y-B.y)*(xa+.5-B.x))*inv, lb=((A.x-C.x)*(py-C.y)-(A.y-C.y)*(xa+.5-C.x))*inv;
        ...
        for(let x=xa;x<=xb;x++, la+=dax, lb+=dbx){
          ...
          if(!dup){
            if(cnt<K){ pf[c*K+cnt]=i; pn[c]=cnt+1; } else { pf.copyWithin(c*K,c*K+1,c*K+K); pf[c*K+K-1]=i; }
            if(dough && pd[c]<255) pd[c]++;
          }
          tf[c]=i;
          th[c]=la*hs[0]+lb*hs[t]+lc*hs[t+1];
  ...
  // Верх сглаживается в пределах материала: мелко (~2 CSS px) — грани сетки иначе видны
  // ступеньками; крупно (~30 CSS px) — при ×5 однослойные места конверта читались ямами, а
  // края клапанов — хребтами (проба 17.09). Остаётся TABLE_RELIEF мелкого рельефа: волна над
  // начинкой; края клапанов видны тенью.
  {
    const m=new Float32Array(N), v=new Float32Array(N);
    for(let c=0;c<N;c++) if(tf[c]>=0){ m[c]=1; v[c]=th[c]; }
    const box=(a,r)=>{ const o=new Float32Array(N), o2=new Float32Array(N);
      for(let y=0;y<bh;y++){ let acc=0; const row=y*bw; for(let x=-r;x<bw;x++){ if(x+r<bw) acc+=a[row+x+r]; if(x-r-1>=0) acc-=a[row+x-r-1]; if(x>=0) o[row+x]=acc; } }
      for(let x=0;x<bw;x++){ let acc=0; for(let y=-r;y<bh;y++){ if(y+r<bh) acc+=o[(y+r)*bw+x]; if(y-r-1>=0) acc-=o[(y-r-1)*bw+x]; if(y>=0) o2[y*bw+x]=acc; } }
      return o2; };
    const r1=Math.max(1,Math.round(2*s)), r2=Math.max(3,Math.round(30*s));
    const v1=box(v,r1), m1=box(m,r1), v2=box(v,r2), m2=box(m,r2);
    for(let c=0;c<N;c++) if(tf[c]>=0 && m1[c]>0) th[c]=TABLE_RELIEF*v1[c]/m1[c]+(1-TABLE_RELIEF)*v2[c]/m2[c];
  }
  // Размер пикселя в долях роти — допуск «у разреза».
  const o0=screen({x:0,y:0}), o1=screen({x:1,y:0}), unit=1/(Math.hypot(o1.x-o0.x,o1.y-o0.y)||1);
  ...
  // Грани далеко от резов не проверяются попиксельно.
  const cutFace=new Uint8Array(n);
  if(cuts.length) for(let i=0;i<n;i++){
    const ps=faces[i].points; let x0=Infinity,x1=-Infinity,y0=Infinity,y1=-Infinity;
    for(const p of ps){ if(p.x<x0)x0=p.x; if(p.x>x1)x1=p.x; if(p.y<y0)y0=p.y; if(p.y>y1)y1=p.y; }
    const cx=(x0+x1)/2, cy=(y0+y1)/2, rad=Math.hypot(x1-x0,y1-y0)/2+rimW+unit*3+.01;
    for(const k of cuts){
      const ax=cx-k.ax, ay=cy-k.ay, along=ax*k.ux+ay*k.uy;
      if(along<-rad || along>k.len+rad) continue;
      if(Math.abs(-ax*k.uy+ay*k.ux)-k.half<rad){ cutFace[i]=1; break; }
    }
  }
  let ncD=0, ncAlong=0;
  const nearCut=(u,v)=>{
  ...
  let UP=1; for(let c=0;c<N;c++) if(th[c]>UP) UP=th[c];
  UP=Math.ceil(UP)+2;
  const padX=Math.ceil(SHADOW_DX*UP)+4, padY=Math.ceil(SHADOW_DY*UP)+4;
  const OW=bw+padX, OH=bh+UP+padY, out=new Uint8ClampedArray(OW*OH*4);
  // Тень: силуэт, сдвинутый на свою высоту (и на половину — без просветов), размытый.
  let sh=new Float32Array(OW*OH);
  for(let j=0;j<bh;j++) for(let i=0;i<bw;i++){
    const c=j*bw+i; if(tf[c]<0) continue;
    const h=th[c];
    let x=Math.round(i+SHADOW_DX*h), y=Math.round(j+UP+SHADOW_DY*h); if(x<OW && y<OH) sh[y*OW+x]=1;
    x=Math.round(i+SHADOW_DX*h*.5); y=Math.round(j+UP+SHADOW_DY*h*.5); if(x<OW && y<OH) sh[y*OW+x]=1;
  }
  const rb=Math.max(1,Math.round(1.5*s));
  const blur=(a,w,h,r)=>{ const o=new Float32Array(a.length);
    for(let y=0;y<h;y++){ let acc=0; for(let x=-r;x<w+r;x++){ if(x+r<w) acc+=a[y*w+x+r]; if(x-r-1>=0) acc-=a[y*w+x-r-1]; if(x>=0 && x<w) o[y*w+x]=acc/(2*r+1); } }
    const o2=new Float32Array(a.length);
    for(let x=0;x<w;x++){ let acc=0; for(let y=-r;y<h+r;y++){ if(y+r<h) acc+=o[(y+r)*w+x]; if(y-r-1>=0) acc-=o[(y-r-1)*w+x]; if(y>=0 && y<h) o2[y*w+x]=acc/(2*r+1); } }
    return o2; };
  sh=blur(sh,OW,OH,rb);
  for(let q=0;q<OW*OH;q++) if(sh[q]>0){ out[q*4]=12; out[q*4+1]=7; out[q*4+2]=3; out[q*4+3]=Math.round(Math.min(1,sh[q]*1.6)*.5*255); }
  ...
  const topColor=(c,i,j,dl,light)=>{
    ...
    if(f.kind==="filling" && f.pc && f.pr>0){
      ...
      // полоска начинки: жёлто-яичная с бледными ломтиками вдоль реза, у самой кромки темнее
      // ломтики неровные: у каждого свой сдвиг, размер, часть мест пустует
    ...
    if(f.kind==="dough" && s>=.5){
      ...
      // (в пиксельных видах A/B пиксель крупнее кромки — там это шум)
  ...
  const wallOf=c=>{
    ...
    return {cut:true,bands:cutWallBands(Array.from(pf.subarray(c*K,c*K+pn[c]),k=>faces[k]),{x:tu[c],y:tv[c]})};
  };
  const wallColor=(w,z,h,i)=>{
    const phi=Math.max(0,Math.min(.9999,z/h)), ao=.84+.16*phi;
    let c;
    if(!w.cut){ if(z<Math.max(1.2*s,.22*h)){ c=w.low; stat.low++; } else c=w.side; }
    else {
      let b=w.bands[w.bands.length-1];
      for(const x of w.bands) if(phi<x.to){ b=x; break; }
      const zf=b.from*h, zt=b.to*h;
      if(b.f.kind==="filling"){
        const ph=(((i%per)+per)%per)/per-.5, dz=(z-(zf+zt)/2)/Math.max(.5,(zt-zf)/2);
        c=(ph/.34)**2+(dz/.62)**2<1 ? flesh : FILL;
        stat.wallFill++;
      } else if(z-zf<cp && zt-zf>2.5*cp){ c=b.c0; stat.crustDown++; }
      else if(zt-z<cp && zt-zf>2.5*cp){ c=b.c1; stat.crustUp++; }
      else c=b.crumb;
      stat.cutWall++;
    }
    R=c[0]*ao; G=c[1]*ao; B=c[2]*ao;
  };
  // Наклон верха → свет слева-сверху-издалека (как у подушки на таве), вполсилы.
  const slopeK=.5*TILT/Math.max(.2,VIEW_UP);
  for(let i=0;i<bw;i++){
    let ymin=Infinity;
    for(let j=bh-1;j>=0;j--){
      ...
      for(let r=rt;r<=rEnd;r++){
        ...
        const q=((r+UP)*OW+i)*4; out[q]=R; out[q+1]=G; out[q+2]=B; out[q+3]=255;
      }
      ymin=rt;
    }
  }
  const t3=performance.now();
  return {x:X0,y:Y0-UP,w:OW,h:OH,up:UP,bw,bh,th,tf,data:out,ms:t3-t0,stages:[t1-t0,t2-t1,t3-t2].map(v=>+v.toFixed(1)),stat,texels:N};
}
function drawDishTable(g,screen,faces,H,zk,sx,sy){
  ...
  if(!C || C.faces!==faces || C.key!==key){
    const R=renderDishTable(faces,sc,H,zk*q,Math.round(W*q),Math.round(Hc*q),s*q);
    C=drawDishTable.cache={faces,key,R,q,drawn:false};
    if(R && typeof document!=="undefined"){
      if(!tableCanvas) tableCanvas=document.createElement("canvas");
      tableCanvas.width=R.w; tableCanvas.height=R.h;
      const tc=tableCanvas.getContext("2d");
      if(tc && tc.createImageData && tc.putImageData){ const img=tc.createImageData(R.w,R.h); img.data.set(R.data); tc.putImageData(img,0,0); C.drawn=true; }
    }
  }
  if(!(C.R && C.drawn)) return;
...
function drawDish(g,sx,sy){
  ...
  if(dishGesture && !slide && dishGesture.zone!=="middle" && !(dishGesture.press && !dishGesture.moved)){
    const d=dishGesture, o=d.cutShift||{x:0,y:0}, sh=q=>screen({x:q.x+o.x,y:q.y+o.y});
...
function onDown(id, cx, cy, t){
  ...
  if(dish){ audioInit(); if(AC && AC.state==="suspended") AC.resume();
            ...
            beginDishGesture(id,p,{x:cx,y:cy},t); return; }
...
window.addEventListener("pointerdown", e=>{
  ...
  if(e.altKey && canTwist() && !dishTwist){
...
function gestureLive(){ return action.state !== "RESTING" || !!dishGesture || !!dishFlight || !!dishMove || !!fatGesture; }
