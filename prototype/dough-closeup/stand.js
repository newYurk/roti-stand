...
const BUILD = "2026-09-17 · правки проверки · 22";
...
// бесконечной прямой реза рвало целый кусок при резе «Т»).
...
function dishPieceShift(faces){
  ...
  if(cache.has(faces)) return cache.get(faces);
  const piece=dishPieces(faces), area=new Map();
  let total=0,tx=0,ty=0;
  faces.forEach((f,i)=>{
    if(f.kind!=="dough") return;
    const a=polyArea(f.points); let cx=0,cy=0; for(const p of f.points){ cx+=p.x; cy+=p.y; } cx/=f.points.length; cy/=f.points.length;
    const e=area.get(piece[i])||{a:0,x:0,y:0}; e.a+=a; e.x+=cx*a; e.y+=cy*a; area.set(piece[i],e);
    total+=a; tx+=cx*a; ty+=cy*a;
  });
  let out=null;
  if(area.size>1 && total>0){
    const c={x:tx/total,y:ty/total}, dir=new Map();
    for(const [k,e] of area){
      const dx=e.x/e.a-c.x, dy=e.y/e.a-c.y, l=Math.hypot(dx,dy);
      dir.set(k,l>1e-6 ? {x:dx/l*PIECE_SPREAD,y:dy/l*PIECE_SPREAD} : {x:0,y:0});
    }
    out=new Map();
    faces.forEach((f,i)=>out.set(f,dir.get(piece[i])||{x:0,y:0}));
  }
...
function drawFatAndButter(g, sx, sy){
  ...
  if(fatMap){
    const cell=2/FAT_N, rr=PAN_R*cell*1.35;   // пятна перекрываются — мазок, а не бусы
    for(let j=0;j<FAT_N;j++) for(let i=0;i<FAT_N;i++){
      const f=fatMap[j*FAT_N+i]; if(f<.04) continue;
      const c=P(panC.x+(-1+(i+.5)*cell)*PAN_R, panC.y+(-1+(j+.5)*cell)*PAN_R);
      g.fillStyle=`rgba(236,206,120,${(.13*f).toFixed(3)})`;
      g.beginPath(); g.ellipse(c.x,c.y,rr*sx,rr*TILT*sy,0,0,Math.PI*2); g.fill();
    }
  }
  const b=butterSpot(), tx=-Math.sin(b.a), ty=Math.cos(b.a), nx=Math.cos(b.a), ny=Math.sin(b.a);
  const L=PAN_R*.09, W=PAN_R*.06, H=PAN_R*.035*TILT;
