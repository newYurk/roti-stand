...
const BUILD = "2026-09-16 · складка внутрь · 5";
...
function beginDishGesture(id,p,client){
  ...
  dishGesture={id,start,end:start,anchor,inward,preview:null,client:at,samples,moved:false};
}
...
function endDishGesture(){
  ...
  const ok=dish.mode==="fold" ? g.target&&inwardFold(g)&&foldDish(g.anchor,g.target) : cutDish(g.start,g.end);
