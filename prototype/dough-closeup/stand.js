...
const BUILD = "2026-09-17 · поворот · 27";
...
function onDown(id, cx, cy, t){
  ...
  if(dish){ audioInit(); if(AC && AC.state==="suspended") AC.resume();
            ...
            if(id[0]==="t" && dishTouches.size===2 && !fatGesture && canTwist()){
              const ids=[...dishTouches.keys()];
              if(beginTwist("touch",{ids})){ dishTwist.raw=tableAngle(dishTouches.get(ids[0]),dishTouches.get(ids[1])); return; }
            }
...
function onCancel(id){
  dishTouches.delete(id);
  if(dishTwist && ((dishTwist.ids && dishTwist.ids.includes(id)) || dishTwist.id===id)){ endTwist(false); return; }
  if(fatGesture && fatGesture.id===id){ fatGesture=null; return; }
