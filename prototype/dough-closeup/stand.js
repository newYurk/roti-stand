...
const BUILD = "2026-09-17 · рама экрана · 36";
...
// поэтому свой мы не рисуем, а зоны берём с неё. Включается адресом `?frame=riverside`.
// Замеры по картинке 1024×576 (сетка, 17.09): полка кончается на y≈235, рабочая поверхность слева
// x 20…450, лоток лопаток 455…560, тава — эллипс (800, 352) с полуосями 228×112, панель с y≈490.
// Картинка — одобренный экран с ПУСТЫМ столом и пустой тавой: свой лист и роти рисует стенд.
// Получена правкой одобренного экрана (Draw Things, 17.09), лежит рядом со стендом, 71 КБ.
// Замеры сделаны по копии 1024×576, поэтому рисуем её в том же логическом размере.
const FRAME_SRC = "frame-riverside.jpg", FRAME_W = 1024, FRAME_H = 576;
const FRAME_WORK = {x:24, y:246, w:420, h:225};      // стальная поверхность под тесто
const FRAME_PAN  = {cx:800, cy:352, rx:228, ry:112}; // тава
const FRAME_BAND = {y0:235, y1:492};                 // играбельная полоса: от полки до нижней панели
let frameImg = null, frameOn = false;
...
function frameWantedSafe(){ try { return frameWanted(); } catch(e) { return false; } }
function frameWanted(){
  try { return new URLSearchParams(location.search).get("frame")==="riverside"; } catch(e) { return false; }
}
function frameFit(){                                  // картинка → холст: по ширине, полоса по центру
  const k = cv.width / FRAME_W;
  const bandMid = (FRAME_BAND.y0 + FRAME_BAND.y1) * 0.5;
...
function zone(name){
  if(frameOn){
    ...
    const W_ = FRAME_WORK, P_ = FRAME_PAN;
    const box = name==="work"
...
function layoutPan(){
  ...
  if(frameOn){                       // в раме тава уже нарисована — садимся ровно в неё
    const c = unproject(frameX(FRAME_PAN.cx), frameY(FRAME_PAN.cy));
    panC = c;
    PAN_R = frameX(FRAME_PAN.rx) - frameX(0);
  }
}
...
function butterSpot(){
  // дальний от стола борт: в портрете — вверх-вправо, в альбоме — вправо
  const a = stackedLayout() ? -35*Math.PI/180 : 20*Math.PI/180;
  return {x:panC.x+Math.cos(a)*PAN_R*.93, y:panC.y+Math.sin(a)*PAN_R*.93, a};
}
