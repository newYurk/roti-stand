...
const BUILD = "2026-09-17 · рельеф на столе · 26";
...
function drawDishTable(g,screen,faces,H,zk,sx,sy){
  ...
  const o=screen({x:0,y:0}), e=screen({x:1,y:1});
  const key=[W,Hc,o.x,o.y,e.x,e.y,zk,s,dish.panTime||0,dish.flips||0,SHOW_THROUGH].map(v=>+(+v).toFixed(3)).join(",");
  let C=drawDishTable.cache;
  if(!C || C.faces!==faces || C.key!==key){
    const R=renderDishTable(faces,screen,H,zk,W,Hc,s);
    C=drawDishTable.cache={faces,key,R,drawn:false};
  ...
  if(C.R && C.drawn) g.drawImage(tableCanvas,C.R.x,C.R.y);
}
